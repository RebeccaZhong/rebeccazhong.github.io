...
- [事务](#事务)
	...
	- [声明事务](#声明事务)
		...
		- [隐式事务](#隐式事务)
		- [分布式事务](#分布式事务)
	- [事务并发的四种隔离级别](#事务并发的四种隔离级别)
		- [1. Serializable（串行化）](#1-serializable串行化)
		- [2. Repeatable-read（可重复读）](#2-repeatable-read可重复读)
		- [3. Read-committed（读已提交）](#3-read-committed读已提交)
		- [4. Read-uncommitted（读未提交）](#4-read-uncommitted读未提交)
		- [事务并发带来的问题](#事务并发带来的问题)
			- [脏读](#脏读)
			- [不可重复读](#不可重复读)
			- [幻读](#幻读)
		- [查看当前事务的隔离级别](#查看当前事务的隔离级别)
		- [设置事务的隔离级别](#设置事务的隔离级别)
	- [事务隔离级别验证](#事务隔离级别验证)
		- [验证不同会话的隔离级别](#验证不同会话的隔离级别)
	- [MySQL中事务实现机制](#mysql中事务实现机制)
		- [事务日志](#事务日志)
	- [MVCC](#mvcc)
- [锁](#锁)
	- [锁级别](#锁级别)
